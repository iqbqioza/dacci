...
<div bind:this={rootEl} class="relative">
  <button
    ...
    onclick={() => (open = !open)}
  >
    <span class="text-gray-900 ">{currentLabel()}</span>
    <svg
  ...
  {#if open}
    <ul
      class="absolute left-0 right-0 z-10 mt-1 max-h-56 overflow-y-auto rounded border border-gray-200 bg-white py-1 shadow-lg  "
    >
      {#each options as option}
        <li>
          ...
          >
            <span class={option.value === value ? "font-medium text-blue-600 " : "text-gray-700 "}>
              {option.label}
            </span>
